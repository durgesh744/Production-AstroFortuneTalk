...
import {api_url, privacy_policy_astrologer} from '../config/Constants';
  ...
  const get_policy = async () => {
    try {
      ...
      const response = await ApiRequests.getRequest({
        url: api_url + privacy_policy_astrologer,
      });
      if (response?.status == 1) {
        updateState({policyData: response?.records[0]?.description});
      }
